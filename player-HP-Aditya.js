...
function createPlaylist(){
    ...
    if(playlists.length===0){
        ...
        temp = playlists[playlists.length-1].id+1; //id of a playlist should be equal to prev song id + 1
    }
    ...
    removeBtn.addEventListener('click',()=>{
        deletePlaylist(playlistObj.playlistName);
    });
...
function updateCurrentPlaylist(){
    ...
    for(let i=0;i<playlists.length;i++){
        if(playlists[i].id === currPlaylistId){
            ind = i;
            break;
        }
    }
    ...
    if(currPlaylistId!=-1){
        console.log(currPlaylistId);
        console.log(ind);
        playlists[ind].playlistSongs.forEach((song)=>{
...
function addSongToPlaylist(){
    if(playlists.length!=0){
        ...
        alert('Looks like its time to create a playlist!');
    }
}
...
function updatePlaylists(){
    ...
    playlists.forEach((playlist)=>{
        ...
        if(playlists.length===0){
            ...
            temp = playlists[playlists.length-1].id+1;
        }
...
playlistNameEl.addEventListener('keydown',(event)=>{
    if(event.key==='Enter'){
